...
# ---------------------------------------------------------------------------
# Recovery metrics (Exp C)
# ---------------------------------------------------------------------------
def rolling_reach_rate(reached_goals, window=10):
    """Compute rolling window reach rate over episodes.

    Args:
        reached_goals: 1D array of 0/1 (reached goal in each episode).
        window: Rolling window size.

    Returns:
        1D array of length len(reached_goals), with the rolling mean
        of the last `window` episodes at each position.
    """
    reached = np.asarray(reached_goals, dtype=float)
    n = len(reached)
    result = np.empty(n)
    for i in range(n):
        start = max(0, i - window + 1)
        result[i] = reached[start:i + 1].mean()
    return result


def recovery_latency(reached_goals, window=10, threshold=0.3):
    """First episode where rolling reach rate >= threshold.

    Args:
        reached_goals: 1D array of 0/1 per episode.
        window: Rolling window size.
        threshold: Reach rate threshold for "recovered".

    Returns:
        Episode index (0-based within the provided array), or
        len(reached_goals) if threshold is never reached.
    """
    rates = rolling_reach_rate(reached_goals, window)
    above = np.where(rates >= threshold)[0]
    if len(above) == 0:
        return len(reached_goals)
    return int(above[0])


def area_under_recovery(reached_goals, window=10):
    """Area under the rolling reach rate curve (normalized to [0, 1]).

    Higher = faster and more sustained recovery. Robust alternative to
