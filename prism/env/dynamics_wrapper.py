...
class DynamicsWrapper(gym.Wrapper):
    ...
    @staticmethod
    def select_passages_to_block(
        passages: list[tuple[int, int]],
        goal_pos: tuple[int, int],
        n_keep: int = 1,
    ) -> tuple[list[tuple[int, int]], list[tuple[int, int]]]:
        """Select which passages to block, keeping those closest to the goal.

        Args:
            passages: List of (x, y) passage positions.
            goal_pos: (x, y) of the current active goal.
            n_keep: Number of passages to keep open (default 1).

        Returns:
            (to_block, to_keep) — two lists of (x, y) positions.
        """
        if n_keep >= len(passages):
            return [], list(passages)

        # Sort by Manhattan distance to goal (ascending = closest first)
        ranked = sorted(
            passages,
            key=lambda p: abs(p[0] - goal_pos[0]) + abs(p[1] - goal_pos[1]),
        )
        to_keep = ranked[:n_keep]
        to_block = [p for p in passages if p not in to_keep]
        return to_block, to_keep

    def find_passages(self) -> list[tuple[int, int]]:
        """Detect passages (openings) in the internal walls of FourRooms.

        Scans the median row (y = height//2) and median column (x = width//2)
        for empty cells — these are the passages between rooms.

        Returns:
            List of (x, y) passage positions.
        """
        grid = self.unwrapped.grid
        mid_x = grid.width // 2
        mid_y = grid.height // 2
        ...
        # Scan horizontal wall (median row)
        for x in range(1, grid.width - 1):
            if x == mid_x:
                continue  # skip intersection
            cell = grid.get(x, mid_y)
            if cell is None:
                passages.append((x, mid_y))

        # Scan vertical wall (median column)
        for y in range(1, grid.height - 1):
            if y == mid_y:
                continue  # skip intersection
            cell = grid.get(mid_x, y)
            if cell is None:
                passages.append((mid_x, y))
